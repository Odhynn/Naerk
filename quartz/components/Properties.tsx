...
function createPropertyElement(key: string, value: any) {
  return(
    <tr>
      <td class="property">{key}</td><td class="value">{value}</td>
    </tr>
  )
}
...
export default (() => {
  function PropertiesWithWorkingLinks({fileData, allFiles}: QuartzComponentProps, ) {
    ...
    if(Object.keys(fileData.frontmatter?.wiki ?? {}).length > 0){
      for (const [key, value] of Object.entries(fileData.frontmatter?.wiki ?? {})) {
    ...
    return (      
      <div class="properties">
        <table>{propertiesElements}</table>
      </div>
    )
